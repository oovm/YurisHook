...
impl Process {
    ...
    pub fn module(&self, name: &str) -> Result<Module, MemoryError> {
        ...
        if !module32first(&h_snap, &mut me32) {
            return Err(MemoryError::IterateSnapshotFailure);
        }
    ...
    /// ```
    pub fn read_mem<T: Default>(&self, address: usize) -> Result<T, MemoryError> {
    ...
    pub fn read_mem_chain<T: Default>(&self, mut chain: Vec<usize>) -> Result<T, MemoryError> {
        ...
        while chain.len() != 1 {
            ...
            address =
                if self.iswow64 { self.read_mem::<u32>(address)? as usize } else { self.read_mem::<u64>(address)? as usize }
        }
