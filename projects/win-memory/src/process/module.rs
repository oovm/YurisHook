...
impl Module {
    ...
    /// ```rust
    /// use proc_mem::{Module, ProcMemError, Process, Signature};
    ...
    /// ```
    pub fn find_signature(&self, sig: &Signature) -> Result<usize, MemoryError> {
        let mut addr = Self::find_pattern(&self.module_data, &sig.pattern).ok_or(MemoryError::SignatureNotFound)?;

        for (_i, o) in sig.offsets.iter().enumerate() {
            let pos = (addr as isize).wrapping_add(*o) as usize;
            let data = self.module_data.get(pos).ok_or_else(|| MemoryError::AddressOutOfBounds)?;
        ...
        if sig.rip_relative {
            ...
            let rip: u32 = self.get_raw(addr, true).ok_or(MemoryError::RIPRelativeFailed)?;
    ...
    fn find_pattern(data: &[u8], pattern: &str) -> Option<usize> {
        Self::generate_regex(pattern).and_then(|r| r.find(data)).and_then(|m| Some(m.start()))
    }
}
